...
namespace NoasDK.UI
{
    ...
    public class SafeArea : MonoBehaviour
    {
        ...
        [Tooltip("Is flip safe area")]
        [SerializeField] bool isFlip = false;  // Conform to screen safe area on Y-axis (default true, disable to ignore)
        [Tooltip("Is safe area for banner")]
        [SerializeField] bool isSetBanner = false;
        [Tooltip("Is Flip Banner")]
        [SerializeField] bool isFlipBanner = false;
        [SerializeField] bool ignoreArea;
        private Canvas canvas;
        public static Rect rect;
        public static Rect refRect;
        private static List<SafeArea> listSafeArea = new List<SafeArea>();
        void Awake()
        {
            listSafeArea.Add(this);
        ...
        public static void RefreshAreaImmediately(bool activeBanner)
        {
            for (int i = 0; i < listSafeArea.Count; i++)
            {
                if (listSafeArea[i] == null) continue;
                if (!listSafeArea[i].gameObject.activeInHierarchy) continue;
                listSafeArea[i].RefreshInstance(activeBanner);
            }
        }
        void Update()
        {
        ...
        public void RefreshInstance(bool activeBanner)
        {
            this.isSetBanner = activeBanner;
            if (this.isSetBanner == false)
            {
                this.isFlipBanner = false;
            }
            Refresh(true);
        }
        void Refresh(bool instance = false)
        {
            Rect safeArea = GetSafeArea();

            if (instance || safeArea != LastSafeArea
        ...
        void ApplySafeArea(Rect r)
        {
            ...
            if (ignoreArea)
            {
                RectTransform rc = UIManager.Instance.canvas.GetComponent<RectTransform>();
                Panel.sizeDelta = rc.sizeDelta;
                Panel.anchorMin = Vector2.one * 0.5f;
                Panel.anchorMax = Vector2.one * 0.5f;
                Panel.pivot = Vector2.one * 0.5f;
                Panel.position = rc.position;
                return;
            }
            ...
            if (Screen.width > 0 && Screen.height > 0)
            {
                ...
                //#elif UNITY_ANDROID
                //                Vector2 anchorMin = new Vector2(r.position.x, 50 * (Screen.dpi / 160.0f));
                //#elif UNITY_IPHONE
                //                Vector2 anchorMin = new Vector2(r.position.x, Screen.width / GameHelper.getScreenWidth() * 50 + r.position.y);
                //#else
                //                Vector2 anchorMin = r.position;
                //#endif
                Vector2 anchorMin = r.position;
                ...
                if (isFlip)
                {
                    ...
                    anchorMax.x = Screen.width / anchorMax.x;
                }
                else
                {
                ...
                if (isSetBanner)
                {
                    var height = DeviceUtility.GetSizeBanner();
                    Panel.anchorMin = new Vector2(0, height / Screen.height);
                }
                else if (isFlipBanner)
                {
                    var height = DeviceUtility.GetSizeBanner();
                    height = height * (canvas.GetComponent<RectTransform>().rect.height / Screen.height);
                    Panel.sizeDelta = new Vector2(0, height);
                    Panel.localPosition = new Vector2(0, -height / 2);
                }
                if (UIManager.Instance != null)
                {
                    refRect = Panel.rect;
                    UIManager.Instance.ConfigSize();
                }
            }
            if (Logging)
            {
